...
勇哥，关于XDLA这四张表的TMG标准化我已经制定一遍了。
http://10.240.0.1/t/topic/36046
这里有 现有表 -> TMG标准化表  字段映射 字段类型转换信息。
以及 TMG标准表 TMG归档合并请求。
需要帮忙确认：
1.从业务组角度识别，转换是否合理，当前的解释等是否合理。
这里需要这边提前整理一下这四张表的每个字段的详细业务背景，以及四张表的设计，明天需要先跨团队对齐表内容，后天需要TMG评审，两次会议中，都会对表的具体业务，细致到字段级别进行详细询问，到时候是需要业务组帮忙回答的。