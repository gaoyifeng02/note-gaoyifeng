把这个整理成一个更易懂的文章，写到doc目录下。

有这么一个场景：
两张表A和B，1 - n的关系。
有一个SQL，查询条件需要同时用到A和B的字段，所以需要先把A和B JOIN 后 ，再查询，且先join再查询，就意味着全表join，资源占用很大。
此问题通过冗余字段解决。把B的字段冗余到A表中，这样就不用先join再查询了。
但是这时候，跑sql发现还是出现了以下问题，带count执行就特别慢：
